...
KNOWN_CLUSTERS = [
    ...
    (47.00, 47.15, 16.80, 17.00, "Kam", "Hungary", "Jeli Arboretum botanical garden, blooming rhododendron and azalea flowers, Vas County"),
    ...
    (49.30, 49.50, 16.60, 16.80, "Blansko", "Czech Republic", "Punkva Caves, Macocha Abyss, Moravian Karst (Moravsky kras) nature reserve"),
...
def resolve_location(lat, lon, default_loc=None):
...
def extract_frames_from_video(video_path, output_dir, max_thumb_dim=640, default_loc=None):
    ...
    loc_info = resolve_location(lat, lon, default_loc=default_loc)
...
def process_video_directory(input_dir, output_dir=None, limit=None, default_loc=None):
    ...
    def _worker(f):
        ...
        res = extract_frames_from_video(vpath, output_dir, default_loc=default_loc)
        return f, res
...
if __name__ == "__main__":
    ...
    parser.add_argument("--city", default=None, help="Default city/location if GPS is not available")
    parser.add_argument("--country", default=None, help="Default country if GPS is not available")
    parser.add_argument("--hint", default=None, help="Default location hint if GPS is not available")

    args = parser.parse_args()
    default_loc = None
    if args.city or args.country or args.hint:
        default_loc = {
            "city": args.city or "Unknown",
            "country": args.country or "Unknown",
            "hint": args.hint or ""
        }
    process_video_directory(args.input, args.output, args.limit, default_loc=default_loc)
